...
export default function SupplierLedger() {
  ...
  return (
    <div>
      <h1 className="ptitle">Supplier Ledger</h1>
      <div className="flex gap-2 mb-3 items-end">
        <label className="lbl">Supplier<select value={sel} onChange={(e) => setSel(e.target.value)} className="w-72">{suppliers.map((s) => <option key={s._id} value={s._id}>{s.accountName} · {s.city}</option>)}</select></label>
      ...
      <div className="card p-0 overflow-auto max-h-60vh">
        <table className="tbl">
          ...
          <tbody>
            <tr><td colSpan={3}>Opening Balance ({account?.balanceType})</td><td></td><td className="mono">₹{inr(opening)}</td></tr>
            {invoices.map((r) => (
              <tr key={r._id}><td>{new Date(r.date).toLocaleDateString('en-IN')}</td><td className="mono">{r.purchaseBillNo}</td><td className="mono">₹{inr(r.totalPurchaseAmount)}</td><td></td><td></td></tr>
            ))}
            <tr><td colSpan={4} className="font-bold">Closing Balance</td><td className="mono font-bold">₹{inr(closing)}</td></tr>
          </tbody>
        </table>
      </div>
    </div>
